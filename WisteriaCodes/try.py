...
trainPath, validPath, trainBboxName, validBboxName, loadModelPath, modelName = args[1], args[2], args[3], args[4], args[5], args[6]
...
model.load_state_dict(torch.load(loadModelPath)) #visualizationのときにもこのbest modelを用いることにする。
...
cpm = CPM(TPRs, FPIs)
rcpm = RCPM(TPRs, FPIs)
print("FROC         val_fauc:{:.4f}  val_cpm:{:.4f}  val_rcpm:{:.4f}".format(fauc, cpm, rcpm))
...
cpm = CPM(TPRs, FPIs)
rcpm = RCPM(TPRs, FPIs)
print("FROC_strict      val_fauc:{:.4f}  val_cpm:{:.4f}  val_rcpm:{:.4f}".format(fauc, cpm, rcpm))
...
cpm = CPM(TPRs, FPIs)
rcpm = RCPM(TPRs, FPIs)
print("FROC_ignoreBigBbox     val_fauc:{:.4f}  val_cpm:{:.4f}  val_rcpm:{:.4f}".format(fauc, cpm, rcpm))
plotFROC(TPRs, FPIs, saveFROCPath + f"FROC_{trainBboxName}_{validBboxName}_{optimizerName}_{variability}_{num_epoch}_ignoreBigBbox.png")

TPRs, FPIs, thresholds = FROC(validloader, model, ignore_big_bbox=True, accept_TP_duplicate=False)
fauc = FAUC(TPRs, FPIs)
cpm = CPM(TPRs, FPIs)
rcpm = RCPM(TPRs, FPIs)
print("FROC_strict_ignoreBigBbox     val_fauc:{:.4f}  val_cpm:{:.4f}  val_rcpm:{:.4f}".format(fauc, cpm, rcpm))
plotFROC(TPRs, FPIs, saveFROCPath + f"FROC_strict_{trainBboxName}_{validBboxName}_{optimizerName}_{variability}_{num_epoch}_ignoreBigBbox.png")
