...
version = int(args[11])
savePath, modelPath, optimizerName = args[12], args[13], args[14] #added
trainDir = "/lustre/gk36/k77012/M2/data/{}/{}/{}/".format(modelPath, optimizerName, trainPath) #modified
validDir = "/lustre/gk36/k77012/M2/data/{}/{}/{}/".format(modelPath, optimizerName, validPath) #modified
...
if version >= 2:
    PATH = "/lustre/gk36/k77012/M2/model/{}/{}/model{}".format(modelPath, optimizerName, version-1)  #version-1 represents the previous step
    model.load_state_dict(torch.load(PATH))

#will be used to check catastrophic forgetting
prevList = []
for i in range(1,version):  #[1,version)
    prevDir = "/lustre/gk36/k77012/M2/data/{}/{}/ver{}_200/".format(modelPath, optimizerName, i)  #"/lustre/gk36/k77012/M2/data/sim{}_200/".format(i)
    df_prev = pd.read_csv("/lustre/gk36/k77012/M2/simDataInfo/bboxInfo/{}/{}/bboxInfo{}_200.csv".format(modelPath, optimizerName, i))  #pd.read_csv("/lustre/gk36/k77012/M2/simDataInfo/bboxInfo/bboxInfo{}_200.csv".format(i))
...
if optimizerName=='VSGD':
    ...
    optimizer = VSGD(model.parameters(), lr=lr, num_iters=num_iters) #VSGD(model.parameters(), lr=lr, variability=variability, num_iters=num_iters, weight_decay=weight_decay)
...
for epoch in range(num_epoch):
    ...
    if optimizerName == "SAM":
        train_loss = trainWithCls(trainloader, model, optimizer)  #SAMの場合はclosureにしないといけないかも
    else:
...
PATH = "/lustre/gk36/k77012/M2/model/{}/{}/model{}".format(modelPath, optimizerName, version)
