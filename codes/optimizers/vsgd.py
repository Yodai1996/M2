            ...
            for p in group['params']:
                ...
                if state['step'] % num_iters != 0:
                    if noise_type == 'Gaussian':
                        ...
                        noise2.mul_(0.).add_(
                            torch.distributions.laplace.Laplace(torch.zeros_like(d_p), variability).sample())
                    elif noise_type == 'Uniform':
                        noise2.mul_(0.).add_(torch.distributions.uniform.Uniform(torch.zeros_like(d_p) - variability,
                                                                                 torch.zeros_like(
                                                                                     d_p) + variability).sample())
