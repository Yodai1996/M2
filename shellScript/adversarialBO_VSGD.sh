...
numIter=2 #4 #30 #4 #30 #4 #iteration for BO
...
do
  ...
  boDir="/lustre/gk36/k77012/M2/bo_io/in/adversarialBO/${optimizer}_variability${variability}_decay${decayRate}"
  mkdir -p ${boDir}
  ...
  #training the model
  python ../codes/train_continual.py ${abnormalDir} ${abnormalDir2} ${testPath} ${bboxPath} ${bboxPath2} ${testBbox} ${modelPath} ${model} ${epoch} ${batch_size} ${numSamples} ${i} ${savePath} ${optimizer} ${variability} ${decayRate} >> ../train_log/adversarialBO/${optimizer}_variability${variability}_decay${decayRate}/model${i}_${trainPath}_${validPath}_${testPath}_${model}_epoch${epoch}_batchsize${batch_size}.txt
  echo $i'_finished'
